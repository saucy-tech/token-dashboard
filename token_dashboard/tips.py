...
def _provider_clause(provider: Optional[str], col: str = "provider") -> Tuple[str, list]:
...
def _affected_sessions(
    ...
    prov_sql, prov_args = _provider_clause(provider)
    rows = c.execute(f"""
      SELECT session_id, COALESCE(provider, 'claude') AS provider, COUNT(*) AS n
        FROM tool_calls
       WHERE {where_sql}{prov_sql}
       GROUP BY session_id, provider
       ORDER BY n DESC
       LIMIT ?
    """, [*args, *prov_args, limit]).fetchall()
...
def _affected_prompts(c, session_id: str, provider: Optional[str] = None, limit: int = 3) -> List[dict]:
    ...
    rows = c.execute(f"""
      SELECT uuid, prompt_text
        FROM messages
       WHERE session_id = ? AND type = 'user' AND prompt_text IS NOT NULL {prov_sql}
       ORDER BY timestamp DESC
       LIMIT ?
    """, (session_id, *prov_args, limit)).fetchall()
    return [
...
def cache_discipline_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    ...
    sql = f"""
      SELECT project_slug,
        ...
        FROM messages
       WHERE type='assistant' AND timestamp >= ? {prov_sql}
       GROUP BY project_slug
       HAVING (cr + rebuild) > 100000
    """
    out = []
    with connect(db_path) as c:
        for row in c.execute(sql, (since, *prov_args)):
...
def repeated_target_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    today_iso = today_iso or datetime.now(timezone.utc).isoformat()
    since = _iso_days_ago(today_iso, 7)
    out = []
    prov_sql, prov_args = _provider_clause(provider)
    with connect(db_path) as c:
        for row in c.execute(f"""
          SELECT target, COUNT(*) AS n, COUNT(DISTINCT session_id) AS sessions,
            ...
            FROM tool_calls
           WHERE tool_name IN ('Read','Edit','Write') AND timestamp >= ? {prov_sql}
        ...
        for row in c.execute(f"""
          SELECT target, COUNT(*) AS n, COUNT(DISTINCT session_id) AS sessions,
                 SUM(COALESCE(result_tokens, 0)) AS result_tokens
            FROM tool_calls
           WHERE tool_name='Bash' AND timestamp >= ? {prov_sql}
...
def right_size_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    today_iso = today_iso or datetime.now(timezone.utc).isoformat()
    since = _iso_days_ago(today_iso, 7)
    prov_sql, prov_args = _provider_clause(provider)
    sql = f"""
      SELECT COUNT(*) AS n,
       ...
       WHERE type='assistant' AND model LIKE '%opus%'
         ...
         AND timestamp >= ? {prov_sql}
    """
    with connect(db_path) as c:
        row = c.execute(sql, (since, *prov_args)).fetchone()
...
def outlier_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    today_iso = today_iso or datetime.now(timezone.utc).isoformat()
    since = _iso_days_ago(today_iso, 7)
    out = []
    prov_tool_sql, prov_tool_args = _provider_clause(provider)
    prov_msg_sql, prov_msg_args = _provider_clause(provider)
    with connect(db_path) as c:
        big = c.execute(f"""
          SELECT COUNT(*) AS n, AVG(result_tokens) AS avg_t,
                 MAX(result_tokens) AS max_t
            FROM tool_calls
           WHERE tool_name='_tool_result' AND result_tokens > 50000 AND timestamp >= ? {prov_tool_sql}
        """, (since, *prov_tool_args)).fetchone()
        ...
        for row in c.execute(f"""
          SELECT agent_id, COUNT(*) AS n,
            ...
            FROM messages
           WHERE is_sidechain=1 AND agent_id IS NOT NULL AND timestamp >= ? {prov_msg_sql}
...
def expensive_pattern_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
...
def doctor_edit_thrashing_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
...
def doctor_error_loop_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
...
def doctor_exploration_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
...
def doctor_abandonment_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    ...
    with connect(db_path) as c:
        rows = c.execute(f"""
          SELECT session_id, project_slug, provider, type, timestamp, prompt_text
            FROM messages
           WHERE timestamp >= ? AND type IN ('user', 'assistant') {prov_sql}
...
def doctor_behavior_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    """Doctor-style user-behavior signals derived from prompt text."""
    today_iso = today_iso or datetime.now(timezone.utc).isoformat()
    since = _iso_days_ago(today_iso, 7)
    user_messages, assistant_counts = _session_message_groups(db_path, since, provider=provider)
    out = []
...
def doctor_rapid_followup_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
...
def doctor_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    """Doctor-inspired quality tips over the same 7-day window."""
    return [
        *doctor_edit_thrashing_tips(db_path, today_iso, provider=provider),
        *doctor_error_loop_tips(db_path, today_iso, provider=provider),
        *doctor_exploration_tips(db_path, today_iso, provider=provider),
        *doctor_abandonment_tips(db_path, today_iso, provider=provider),
        *doctor_behavior_tips(db_path, today_iso, provider=provider),
        *doctor_rapid_followup_tips(db_path, today_iso, provider=provider),
    ]


def all_tips(db_path, today_iso: Optional[str] = None, provider: Optional[str] = None) -> List[dict]:
    return [
        *cache_discipline_tips(db_path, today_iso, provider=provider),
        *repeated_target_tips(db_path, today_iso, provider=provider),
        *right_size_tips(db_path, today_iso, provider=provider),
        *outlier_tips(db_path, today_iso, provider=provider),
        *expensive_pattern_tips(db_path, today_iso, provider=provider),
        *doctor_tips(db_path, today_iso, provider=provider),
    ]
