    ...
    <div class="card">
      ...
      <div class="settings-group">
        ...
        <div class="settings-grid">
          ...
          <label>
            <span>Default hourly token limit</span>
            <input id="hourly-limit" data-limit-input type="number" min="0" step="1000" inputmode="numeric" value="${limits.hourly_tokens ?? ''}" placeholder="300000">
          </label>
          <label>
      ...
      <div class="settings-group provider-surface provider-claude">
        <h4>Anthropic / Claude overrides</h4>
        ...
        <div class="settings-grid">
          ...
          <label>
            <span>Claude hourly override</span>
            <input id="claude-hourly-limit" data-limit-input type="number" min="0" step="1000" inputmode="numeric" value="${limits.providers.claude.hourly_tokens ?? ''}" placeholder="inherit global">
          </label>
          <label>
      ...
      <div class="settings-group provider-surface provider-codex">
        <h4>OpenAI / Codex overrides</h4>
        <p class="muted" style="margin:-4px 0 10px">Optional values. Leave blank to inherit global defaults.</p>
        <div class="settings-grid">
          <label>
            <span>OpenAI/Codex session override</span>
          ...
          <label>
            <span>OpenAI/Codex hourly override</span>
            <input id="codex-hourly-limit" data-limit-input type="number" min="0" step="1000" inputmode="numeric" value="${limits.providers.codex.hourly_tokens ?? ''}" placeholder="inherit global">
          </label>
          <label>
            <span>OpenAI/Codex weekly override</span>
...
function applyPreset(key) {
  ...
  $('#hourly-limit').value = String(preset.hourly_tokens);
...
function validateLimitForm() {
  ...
  const globalHourly = parseOptionalPositive($('#hourly-limit').value);
  ...
  const claudeHourly = parseOptionalPositive($('#claude-hourly-limit').value);
  ...
  const codexHourly = parseOptionalPositive($('#codex-hourly-limit').value);
  ...
  if (Number.isNaN(globalSession) || Number.isNaN(globalHourly) || Number.isNaN(globalWeekly) || Number.isNaN(claudeSession) || Number.isNaN(claudeHourly) || Number.isNaN(claudeWeekly) || Number.isNaN(codexSession) || Number.isNaN(codexHourly) || Number.isNaN(codexWeekly)) {
...
function renderEffectivePreview(payload) {
  $('#effective-preview').innerHTML = [
    effectiveProviderCopy(payload, 'claude', 'Anthropic / Claude'),
    effectiveProviderCopy(payload, 'codex', 'OpenAI / Codex'),
  ].join('');
}
...
function effectiveProviderCopy(payload, providerKey, label) {
  ...
  const hourlyLimit = provider.hourly_tokens || payload.hourly_tokens || null;
  ...
  const hourlySource = provider.hourly_tokens ? `${label} override` : (payload.hourly_tokens ? 'global default' : 'not configured');
  ...
  return `
    <div class="effective-card ${providerKey === 'claude' ? 'provider-claude' : 'provider-codex'}">
      ...
      <div>Hourly limit: <b>${hourlyLimit ? fmtCompact(hourlyLimit) : 'not set'}</b> (${hourlySource})</div>
      <div>Weekly limit: <b>${weeklyLimit ? fmtCompact(weeklyLimit) : 'not set'}</b> (${weeklySource})</div>
      <div class="muted">Effective result: ${label} will use ${sessionLimit ? fmtCompact(sessionLimit) : 'no'} session threshold, ${hourlyLimit ? fmtCompact(hourlyLimit) : 'no'} hourly threshold, and ${weeklyLimit ? fmtCompact(weeklyLimit) : 'no'} weekly threshold.</div>
    </div>`;
}
